import matplotlib.pyplot as plt # 시각화 하기 위해서 필요
...
# KNN(k-nearest neighbors; 최근접 이웃 알고리즘) 알고리즘 사용
# 가장 가까운 데이터를 참고하여 다수결 원칙으로 예측

# 데이터 준비 📊
length = bream_length + smelt_length
weight = bream_weight + smelt_weight
fish_data = [[l, w] for l, w in zip(length, weight)] # 두 리스트를 동시에 순회 하면서 쌍으로 묶어줌
