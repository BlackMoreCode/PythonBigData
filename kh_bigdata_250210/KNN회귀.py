...
# 과대 적합 vs 과소 적합
# 과대 적합: 모델이 훈련 세트에 너무 맞춰져 새로운 데이터에 대한 예측력이 떨어지는 경우
# - 훈련 세트 점수는 높지만 테스트 세트 점수가 낮게 나옴
# 과소 적합: 모델이 충분히 훈련 되지 않아서 데이터 패턴을 잘 학습하지 못한 경우
...
# 모델 개선: 이웃 수 변경
# 과소 적합을 해결하기 위해서는 모델의 복잡도를 높여야하고,
# 복잡도를 높이기 위해서는 k(이웃의 개수)를 줄이면 복잡도 증가
knr.n_neighbors = 3
knr.fit(train_input, train_target)

# 훈련 세트 점수 (조정)
train_score = knr.score(train_input, train_target)
print(f"조정된 훈련 세트 결정계수(R^2) : {train_score}")
# 테스트 세트 점수 (조정)
test_score = knr.score(test_input, test_target)
print(f"조정된 테스트 세트 결정계수(R^2) : {test_score}")